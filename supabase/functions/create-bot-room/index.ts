...
// Creates a 2-player room where seat 1 is a bot (see bot-move). The bot
// is marked ready immediately so the human host can start without
...
Deno.serve((req) => handle(req, async (req) => {
  ...
  if (botErr) throw new HttpError(500, botErr.message);
